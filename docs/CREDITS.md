...
| `assets/sprites/oga-swm-*.png` (17 files: mainchar, tiles, fx, objects/enemies, brainguy boss, backgrounds, shop UI) | not yet wired — pending a future animation-mapping session | platform confirmed, exact page unknown | site-level attestation only for now; `oga-swm-` strongly suggests an OpenGameArt submission, but this repo should not claim an exact page/license row until it is re-found and documented explicitly |
