...
- `python scripts/project-status.py` is the canonical status-generation command.
- Any committed point-in-time status dump belongs under `docs/archive/historical/legacy-imports/`.
- Keep root docs lean for submission; do not maintain a long-lived `docs/STATUS.txt` mirror.
