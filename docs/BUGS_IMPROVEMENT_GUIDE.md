...
**Status note (2026-07-13, reconfirmed 2026-07-14, envelope re-tuned 2026-07-15):** Already fixed in an earlier session; table was stale. Verified fixed by running the real audit, not just reading code. Simulation cross-check (2026-07-14) found the envelope constants still hold under a more rigorous check than the original analytic derivation used. The 2026-07-15 Space Marine Overhaul deliberately raised those same constants further (see ADR-025) - status stays "Fixed," the audit still shows 0 dead-ends, and the constant change's only effect was a documented, bounded reclassification of 2 previously-gated items.
...
| CR-023 | `lib/game/world.ts` | A handful of rooms (`R06` row 16, `R07` row 9, `R12`/`R16`/`R24` row 16) have a single interior row missing its left and/or right border-wall tile, found via a headroom-clearance audit run while widening doors for the Space Marine Overhaul (ADR-025). | Low | 🟡 Open — pre-existing, predates this sprint (not caused by the hitbox change). The gap creates a 1-tile-wide pocket that's very likely unreachable in normal play (the row is otherwise open with no floor beneath most of its width, so a player can't walk to it - the only path in would be a pixel-perfect fall onto a 16px-wide edge ledge). Not fixed here: out of this sprint's blast radius and low-confidence that it's actually reachable; flagged for a dedicated pass if it turns out to matter. |
