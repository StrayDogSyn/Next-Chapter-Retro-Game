...
>
> **Last updated:** 2026-07-14 (Sprint A stability/security pass — CR-002/003/005/008/010/012 verified already fixed, none needed new code; see `docs/SESSION_LOG.md`)
...
| [CR-001](#cr-findings-2026-07-14) | Code Review | Thirteen logic/resource/API/edge-case findings from main-branch review | High | 🟡 Partial — 6 of 13 verified fixed 2026-07-14 (CR-002/003/005/008/010/012); 7 remain open |
...
| CR-004 | `components/GameCanvas.tsx` | Fullscreen `requestFullscreen()` / `exitFullscreen()` promises are not caught, so a blocked call can throw an uncaught rejection. | Low | Not in this sprint's scope, but noted in passing: `toggleFullscreen()` (`components/GameCanvas.tsx` line 164-177) already wraps both calls in try/catch. Appears already fixed too — not formally verified this sprint, left open pending a dedicated pass. |
...
| CR-007 | `lib/game/rng.ts` | `Rng.weighted()` silently returns `undefined` when total weight is zero. | Low | Not in this sprint's scope. Commit `7fe645a`'s message claims "Rng.weighted() guards against empty/invalid weights" — unverified this sprint, left open pending a dedicated check rather than trusting the commit message alone. |
...
| CR-009 | `components/GameCanvas.tsx`, `lib/game/game.ts` | Menu input is handled in two layers (`GameCanvas` polls `input.menu` and `Game`/`InputManager` also flushes it), risking double-toggle or missed presses. | Low | Not in this sprint's scope — still open. |
...
| CR-013 | `lib/game/player-identity.ts` | `getOrCreatePlayerId()` silently gives up and returns `''` if `crypto.randomUUID` is unavailable. | Low | Not in this sprint's scope. Commit `7fe645a`'s message claims a fallback UUID generator was added — unverified this sprint, left open pending a dedicated check. |
