...
// Space Marine Overhaul: game.ts's player hitbox grew across two passes,
// 14x26 -> 18x32 -> 24x44 (ph is now 2.75 tiles). ensureExitClearance() in
// levelLoader.ts was widened to match, 2 -> 3 -> now a 4-tile minimum
// portal, so the enlarged hitbox gets a full 1.25-tile margin passing
// through inter-room exits instead of nearly filling the gap. This test
// verifies that widening actually landed for every declared exit in the
// real, loaded world - not just that the carve function's line count
// changed.
describe("door clearance (Space Marine Overhaul)", () => {
  ...
  it("every declared exit has at least a 4-tile-wide open portal on its edge", () => {
    ...
    for (const room of world.values()) {
      for (const edge of ["left", "right", "up", "down"] as const) {
        ...
        expect(maxRun, `Room ${room.id}'s ${edge} exit has only ${maxRun}-tile clearance`).toBeGreaterThanOrEqual(4);
      }
    }
  });
